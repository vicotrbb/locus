# Experiment 0207: Remote-Free Runtime Window Collection
...
Postulate:
`documentation/postulates/0199-remote-free-runtime-window-collection.md`

The postulate said that a service-window collection helper can gather runtime
summaries from registered owners through short mutable owner borrows, then
route those summaries through the shared service-window runner without
changing guarded retune behavior.

## Change

Added `RemoteFreeServiceRuntimeRetuneOwners::collect_service_window` and
`RemoteFreeServiceRuntimeWindowCollectionError`.

The helper accepts owner IDs, borrows each owner only while the caller-provided
collector builds a `RemoteFreeServiceRetuneSummary`, routes the summary through
`observe_service_window`, merges returned service-window stats, and preserves
owner ID context for missing owners, collector failures, and window-routing
failures.

Extended the service-window benchmark harness with
`remote_free_service_runtime_window_collection_sequence`. It runs the same
real allocation sequence as Experiment 0206, but summary collection happens
inside `collect_service_window`.
...
cargo bench -p locus-alloc --bench remote_free_service_telemetry remote_free_service_runtime_window_collection_sequence -- --sample-size 10 --warm-up-time 0.1 --measurement-time 0.1
...
All validation commands passed. The focused service-window test run reported 6
service-window tests passing. The workspace test run reported 276 unit tests, 1
...
Runtime window collection sample:
...
| Case | Time range |
| --- | ---: |
| `remote_free_service_runtime_window_collection_sequence` | 195.91 to 199.37 us |

## Interpretation

The postulate survived the focused test and benchmark pass.

The collection helper preserved the routed service-window allocation counters:
...
This gives service loops one reusable primitive for collecting owner telemetry
and routing it through the shared service mutation budget without manually
holding and releasing owner borrows at each call site.

## Next Question

Should live service loops collect summaries from every registered owner each
window, or should collection be driven by a dirty-owner queue to avoid scanning
owners that have no new remote-free activity?

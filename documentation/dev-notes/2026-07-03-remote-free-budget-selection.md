...
Experiment 0270 added compact JSON verdicts for verifier-summary drift verdict
rollup checks. The real matched check emits `status=matched` with `drift=null`,
while a controlled stale `records=1` archived rollup emits `status=drifted`
with `drift.field=records`, expected `2`, and actual `1`.

Experiment 0271 added parsers for verifier-summary drift verdict rollup check
JSON. The real matched check artifact reloads as `status=matched`, while the
controlled stale `records=1` artifact reloads as `status=drifted` with
`field=records`, expected `2`, and actual `1`. The parser rejects status drift,
drift payload drift, and nested grouped rollup drift.

Experiment 0272 added a dashboard rollup over saved verifier-summary drift
...
Experiment 0281 added a repeated-check CLI parse alias and focused parser tests
for repeated-check rollup drift verdict rollup check JSON. The real matched
artifact reloads as `status=matched`, while the controlled stale `records=1`
artifact reloads as `status=drifted` with `field=records`.
...
- Do not treat a saved repeated-check rollup drift verdict rollup check rollup
  as opaque text. Experiment 0283 showed the real rollup JSON artifact reloads
  through the repeated-check CLI parser with the same matched, drifted, and
  `records` drift counters.
- Do not accept a parsed repeated-check rollup drift verdict rollup check
  rollup without recomputing it from the source verdict records. Experiment
  0284 showed a controlled stale `records=1` rollup fails strict verification
  with `CountDrift` against expected `records=2`.
- Do not rely only on console text for repeated-check rollup drift verdict
  rollup check rollup drift checks. Experiment 0285 showed the same checks
  emit compact JSON verdict artifacts for matched and controlled stale
  `records=1` archives.
- Do not store repeated-check rollup drift verdict rollup check rollup drift
  verdict JSON without a parser path. Experiment 0286 showed the matched and
  controlled stale `records=1` JSON artifacts reload as typed reports through
  the repeated-check CLI path.
- Do not inspect saved repeated-check rollup drift verdict rollup check rollup
  drift verdict JSON artifacts one at a time when dashboard status is needed.
  Experiment 0287 showed the mixed verdict JSON log aggregates matched,
  drifted, and `records` drift counts through the repeated-check CLI rollup
  path.
- Do not treat a saved repeated-check rollup drift verdict rollup check rollup
  drift verdict rollup as opaque text. Experiment 0288 showed the real rollup
  JSON artifact reloads through the repeated-check CLI parser with the same
  matched, drifted, and `records` drift counters.
- Do not accept a parsed repeated-check rollup drift verdict rollup check
  rollup drift verdict rollup without recomputing it from source verdict
  records. Experiment 0289 showed a controlled stale `records=1` rollup fails
  strict verification with `CountDrift` against expected `records=2`.
- Do not rely only on console text for repeated-check rollup drift verdict
  rollup check rollup drift verdict rollup drift checks. Experiment 0290
  showed the same checks emit compact JSON verdict artifacts for matched and
  controlled stale `records=1` archives.
- Do not store repeated-check rollup drift verdict rollup check rollup drift
  verdict rollup drift verdict JSON without a parser path. Experiment 0291
  showed the matched and controlled stale `records=1` JSON artifacts reload as
  typed reports through the repeated-check CLI path.
- Do not inspect saved repeated-check rollup drift verdict rollup check rollup
  drift verdict rollup drift verdict JSON artifacts one at a time when
...
- Do not accept a parsed repeated-check rollup drift verdict rollup check
  rollup drift verdict rollup drift verdict rollup without recomputing it from
  source verdict records. Experiment 0294 showed a controlled stale
  `records=1` rollup fails strict verification with `CountDrift` against
  expected `records=2`.
- Do not rely only on console text for repeated-check rollup drift verdict
  rollup check rollup drift verdict rollup drift verdict rollup drift checks.
  Experiment 0295 showed the same checks emit compact JSON verdict artifacts
  for matched and controlled stale `records=1` archives.
- Do not store repeated-check rollup drift verdict rollup check rollup drift
  verdict rollup drift verdict rollup drift verdict JSON without a parser path.
  Experiment 0296 showed the matched and controlled stale `records=1` JSON
  artifacts reload as typed reports through the repeated-check CLI path.
- Do not inspect saved repeated-check rollup drift verdict rollup check rollup
  drift verdict rollup drift verdict rollup drift verdict JSON artifacts one
  at a time when dashboard status is needed. Experiment 0297 showed the mixed
  verdict JSON log aggregates matched, drifted, and `records` drift counts
  through the repeated-check CLI rollup path.
- Do not treat saved repeated-check rollup drift verdict rollup check rollup
  drift verdict rollup drift verdict rollup drift verdict rollup JSON as
  one-off console output. Experiment 0298 showed the real dashboard rollup JSON
  reloads as typed counters through the repeated-check CLI parser path.
- Do not accept a parsed repeated-check rollup drift verdict rollup check
  rollup drift verdict rollup drift verdict rollup drift verdict rollup without
...
- Do not store repeated-check dashboard archive drift verdict JSON without a
  parser path. Experiment 0301 showed the matched and controlled stale
  `records=1` JSON artifacts reload as typed reports through the repeated-check
  CLI parser path.
- Do not inspect saved repeated-check dashboard archive drift verdict JSON
  artifacts one at a time when dashboard status is needed. Experiment 0302
  showed the mixed verdict JSON log aggregates matched, drifted, and `records`
  drift counts through the repeated-check CLI rollup path.
- Do not treat saved repeated-check dashboard archive drift verdict rollup JSON
  as opaque text. Experiment 0303 showed the real rollup JSON artifact reloads
  through the repeated-check CLI parser with the same matched, drifted, and
  `records` drift counters.
- Do not accept a parsed repeated-check dashboard archive drift verdict rollup
  without recomputing it from saved archive drift verdict records. Experiment
  0304 showed a controlled stale `records=1` rollup fails strict verification
  with `CountDrift` against expected `records=2`.
...
- `documentation/experiments/0283-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-parser.md`
- `documentation/experiments/0284-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift.md`
- `documentation/experiments/0285-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json.md`
- `documentation/experiments/0286-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-parser.md`
- `documentation/experiments/0287-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup.md`
- `documentation/experiments/0288-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-parser.md`
- `documentation/experiments/0289-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift.md`
...
- `documentation/experiments/0294-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift.md`
- `documentation/experiments/0295-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json.md`
- `documentation/experiments/0296-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-parser.md`
...
- `documentation/experiments/0301-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-parser.md`
- `documentation/experiments/0302-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup.md`
- `documentation/experiments/0303-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-parser.md`
- `documentation/experiments/0304-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift.md`
- `documentation/experiments/0305-remote-free-service-telemetry-rollup-check-log-summary-verdict-rollup-verification-summary-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json.md`
- `documentation/experiments/0306-remote-free-service-telemetry-rollup-drift-json-parser.md`
- `documentation/experiments/0307-remote-free-service-telemetry-rollup-drift-json-rollup.md`
- `documentation/experiments/0308-remote-free-service-telemetry-rollup-drift-json-rollup-parser.md`
- `documentation/experiments/0309-remote-free-service-telemetry-rollup-drift-json-rollup-drift.md`
- `documentation/experiments/0310-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json.md`
...
- `documentation/experiments/0315-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-parser.md`
- `documentation/experiments/0316-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup.md`
- `documentation/experiments/0317-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-parser.md`
- `documentation/experiments/0318-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift.md`
- `documentation/experiments/0319-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json.md`
- `documentation/experiments/0320-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-parser.md`
- `documentation/experiments/0321-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup.md`
- `documentation/experiments/0322-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-parser.md`
- `documentation/experiments/0323-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift.md`
- `documentation/experiments/0324-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json.md`
- `documentation/experiments/0325-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-parser.md`
- `documentation/experiments/0326-remote-free-service-telemetry-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup-drift-json-rollup.md`
...
the validator before treating an archive drift dashboard record as durable.
Experiment 0306 made that parser-only recheck explicit for both matched and
stale saved archive drift verdict rollup drift verdict records.
Experiment 0307 then aggregated those saved matched and stale JSON verdict
records into a compact dashboard rollup with one matched record, one drifted
record, and one `records` drift bucket.
Experiment 0308 made the cohort rollup artifact itself reloadable through the
parser-only path, preserving the same two-record mixed cohort counters.
Experiment 0309 then verified that archived cohort rollup JSON against saved
source verdict records and rejected a controlled stale `records=1` archive
with `CountDrift`.
Experiment 0310 preserved both the matched and stale cohort rollup drift check
outcomes as compact JSON verdicts, then reloaded them through the parser path.
Experiment 0311 made that parser-only recheck explicit for the saved matched
and stale cohort rollup drift check JSON verdict records.
Experiment 0312 aggregated those saved matched and stale JSON verdict records
...
Experiment 0315 made that parser-only recheck explicit for the saved matched
and stale rollup recheck JSON verdict records.
Experiment 0316 aggregated those saved matched and stale rollup recheck JSON
verdict records into a compact dashboard rollup with one matched record, one
drifted record, and one `records` drift bucket.
Experiment 0317 made that dashboard rollup artifact itself reloadable through
the parser-only path, preserving the same two-record rollup recheck cohort
counters.
Experiment 0318 then verified that archived rollup recheck cohort rollup JSON
against saved source verdict records and rejected a controlled stale
`records=1` archive with `CountDrift`.
Experiment 0319 preserved both the matched and stale rollup recheck cohort
rollup drift check outcomes as compact JSON verdicts, then reloaded them
through the parser path.
Experiment 0320 made that parser-only recheck explicit for the saved matched
and stale rollup recheck cohort rollup drift JSON verdict records.
Experiment 0321 aggregated those saved matched and stale JSON verdict records
into a compact dashboard rollup with one matched record, one drifted record,
and one `records` drift bucket.
Experiment 0322 made that dashboard rollup artifact itself reloadable through
the parser-only path, preserving the same two-record rollup recheck cohort
counters.
Experiment 0323 then verified that archived aggregate rollup JSON against
saved source verdict records and rejected a controlled stale `records=1`
aggregate rollup with `CountDrift`.
Experiment 0324 preserved both the matched and stale aggregate rollup recheck
outcomes as compact JSON verdicts, then reloaded them through the parser path.
Experiment 0325 made that parser-only recheck explicit for the saved matched
and stale aggregate rollup recheck JSON verdict records.
...
Experiment 0333 preserved both the matched and stale aggregate rollup recheck
cohort rollup recheck verdict rollup drift outcomes as compact JSON verdicts,
then reloaded them through the parser path.
...
into a compact dashboard rollup with one matched record, one drifted record,
and one `records` drift bucket.
Experiment 0341 made that saved dashboard rollup artifact itself reloadable
...
parser path.
Experiment 0344 made that parser-only recheck explicit for the saved matched
and stale archived dashboard rollup recheck JSON verdict records.
...
Experiment 0350 aggregated those saved matched and stale JSON verdict records
into a compact dashboard rollup with one matched record, one drifted record,
and one `records` drift bucket.

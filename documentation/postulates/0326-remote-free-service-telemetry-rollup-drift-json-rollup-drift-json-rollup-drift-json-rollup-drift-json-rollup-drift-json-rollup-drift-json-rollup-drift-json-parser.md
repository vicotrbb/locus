# Postulate 0326: Remote-Free Service Telemetry Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON Parser
...
Experiment 0333 proved that matched and controlled stale aggregate rollup
recheck cohort rollup recheck verdict rollup drift outcomes emit compact JSON
verdict records. Release dashboards need those saved records to be reloadable
later without the original source verdict logs, preserving both the matched
archive decision and the stale `records` drift decision.

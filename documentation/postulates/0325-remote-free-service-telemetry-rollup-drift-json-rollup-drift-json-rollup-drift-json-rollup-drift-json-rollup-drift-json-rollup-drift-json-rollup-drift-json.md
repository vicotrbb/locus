# Postulate 0325: Remote-Free Service Telemetry Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON Rollup Drift JSON
...
Experiment 0332 proved that archived aggregate rollup recheck cohort rollup
recheck verdict rollup JSON verifies against saved source verdict records and
rejects a controlled stale `records=1` archive. Release dashboards also need a
compact, machine-readable verdict record for those matched and stale recheck
decisions, plus a parser path that can reload the saved verdict record later.
...
```text
remote_free_service_telemetry_summary_validate --rollup-check-json-summary-verdict-rollup-verify-against-json-summary-verify-against-json-rollup-verify-against-json-rollup-verify-against-json <saved-json-verdict-log.txt> <stale-rollup-log.txt>
```

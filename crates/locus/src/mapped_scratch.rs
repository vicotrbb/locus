...
impl MappedScratchArena {
    ...
    pub fn bind_to_node(&self, node: NodeId) -> Result<(), MappedScratchAllocError> {
        crate::sys::linux::bind_region_to_node(&self.region, node.0)
    ...
    pub fn advise_transparent_huge_pages(
        ...
        let sys_advice = match advice {
            MappedScratchHugePageAdvice::HugePage => {
                crate::sys::linux::LinuxTransparentHugePageAdvice::HugePage
            }
            MappedScratchHugePageAdvice::NoHugePage => {
                crate::sys::linux::LinuxTransparentHugePageAdvice::NoHugePage
            }
        };

        crate::sys::linux::advise_region_transparent_huge_pages(&self.region, sys_advice)
...
pub enum MappedScratchAllocError {
    ...
    #[cfg(target_os = "linux")]
    LinuxNumaPolicy(crate::sys::linux::LinuxNumaPolicyError),
    /// Linux transparent huge page advice failed.
    #[cfg(target_os = "linux")]
    LinuxTransparentHugePageAdvice(crate::sys::linux::LinuxTransparentHugePageAdviceError),
}
    ...
    fn mapped_scratch_arena_rejects_invalid_bind_node() {
        ...
        assert!(matches!(
            ...
            MappedScratchAllocError::LinuxNumaPolicy(
                crate::sys::linux::LinuxNumaPolicyError::InvalidNode(4096)
            )
        ));
    }
}

...
//!
//! This module is the single allowed home for unsafe code: operating-system calls and raw memory
...
pub mod linux {
    ...
    pub fn bind_region_to_node(
        ...
        // SAFETY: the region pointer and length come from a live owned
        // mapping, the mask buffer outlives the call, and max_node bounds the
        // bits the kernel may read from it.
        let rc = unsafe {
            libc::syscall(
                libc::SYS_mbind,
    ...
    pub fn advise_region_transparent_huge_pages(
        ...
        // SAFETY: the pointer and length describe one live owned mapping and
        // madvise does not invalidate it.
        let rc = unsafe {
            libc::madvise(
                region.as_ptr().cast::<libc::c_void>(),
                region.len(),
...
impl MappedRegion {
    ...
    pub fn anonymous(len: usize) -> Result<Self, MappedRegionError> {
        ...
        // SAFETY: a NULL-hint anonymous private mapping with a non-zero
        // length dereferences nothing; the result is checked for MAP_FAILED
        // before use.
        let raw = unsafe {
            libc::mmap(
                std::ptr::null_mut(),
    ...
    pub fn as_slice(&self) -> &[u8] {
        // SAFETY: ptr/len describe a live owned mapping that stays valid for
        // the borrow; shared access cannot race a mutation through &self.
    ...
    pub fn as_mut_slice(&mut self) -> &mut [u8] {
        // SAFETY: ptr/len describe a live owned mapping and &mut self makes
        // this the only access for the borrow's lifetime.
    ...
    pub fn write_touch_pages(&mut self, page_size: usize) -> Result<usize, TouchPagesError> {
        ...
        while offset < self.len {
            // SAFETY: offset < self.len keeps the pointer inside the owned
            // mapping, and &mut self guarantees exclusive access.
            unsafe {
    ...
    pub fn lock_pages(&self) -> Result<(), PageLockError> {
        // SAFETY: ptr/len describe a live owned mapping; mlock reads no
    ...
    pub fn unlock_pages(&self) -> Result<(), PageLockError> {
        // SAFETY: ptr/len describe a live owned mapping; munlock reads no
        // memory through the pointer.
...
impl Drop for MappedRegion {
    fn drop(&mut self) {
        // SAFETY: Drop owns the mapping and runs once, so the region is
        // still mapped and never touched again after munmap.

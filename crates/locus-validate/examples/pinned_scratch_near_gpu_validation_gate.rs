...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let program = args
        .next()
        .unwrap_or_else(|| "pinned_scratch_near_gpu_validation_gate".to_owned());
    ...
    let gate = evaluate_pinned_scratch_near_gpu_validation_output(&pinned_scratch_output)?;
    println!("{gate}");
...
fn usage_error(program: &str) -> std::io::Error {
    std::io::Error::new(
        ...
        format!("usage: {program} <near-gpu-pinned-scratch-output>"),
    )
}

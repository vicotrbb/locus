...
struct KvPolicyCase {
    ...
    expected: ExpectedKvPolicyDrift,
}

#[derive(Debug, Clone, Copy)]
struct ExpectedKvPolicyDrift {
...
impl KvPolicyCase {
    fn end_drain() -> Self {
        Self {
            ...
            drift_config: kv_queued_byte_config(),
            expected: ExpectedKvPolicyDrift {
    ...
    fn max_queued256kib() -> Self {
        let config = kv_queued_byte_config();
        Self {
...
fn kv_queued_byte_config() -> RemoteFreeQueuedByteDrainConfig {
...
fn kv_remote_free_tracker_benchmark(c: &mut Criterion, name: &'static str, case: KvPolicyCase) {
    c.bench_function(name, |bench| {
        ...
        bench.iter(|| {
            ...
            assert_kv_policy_drift(case, stats);
            black_box(stats);
        });
...
fn assert_kv_policy_drift(case: KvPolicyCase, stats: KvPolicyStats) {
    assert_eq!(
        stats.max_pending_over_target,

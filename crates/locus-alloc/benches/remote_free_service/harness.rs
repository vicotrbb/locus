...
pub(crate) struct ServiceTelemetryCase {
    label: &'static str,
...
#[derive(Debug, Clone, Copy)]
pub(crate) struct ServiceTelemetryStats {
    ...
    pub(crate) summary: RemoteFreeServiceRetuneSummary,
}

#[derive(Debug, Clone, Copy)]
pub(crate) struct CounterSummary {
    pub(crate) min: u64,
    pub(crate) max: u64,
    sum: u64,
}
...
impl ServiceTelemetryCase {
    pub(crate) fn fixed_policy_all_clean() -> Self {
        Self {
    ...
    pub(crate) fn one_end_drain_owner() -> Self {
        Self {
    ...
    pub(crate) fn planner_candidate_drain_earlier() -> Self {
        Self {
    ...
    pub(crate) fn one_capacity128_end_drain_owner() -> Self {
        Self {
    ...
    pub(crate) fn planner_candidate_capacity_and_drain_earlier() -> Self {
        Self {
...
impl CounterSummary {
    ...
    pub(crate) fn mean_milli(self, samples: u64) -> u64 {
...
pub(crate) fn assert_service_telemetry(case: ServiceTelemetryCase, stats: ServiceTelemetryStats) {
    let counts = stats.summary.action_counts();
...
pub(crate) fn format_milli(value: u64) -> String {
    format!("{}.{:03}", value / 1000, value % 1000)
}

pub fn benchmark_service_telemetry(c: &mut Criterion) {
    remote_free_service_telemetry_fixed_policy(c);
    remote_free_service_telemetry_one_drifting_owner(c);
    remote_free_service_telemetry_planner_candidate_drain_earlier(c);
    remote_free_service_telemetry_one_capacity128_end_drain_owner(c);
    remote_free_service_telemetry_planner_candidate_capacity_and_drain_earlier(c);
}

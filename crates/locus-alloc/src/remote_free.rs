//! Owner-drained remote-free transport.
//!
//! The validated path is chunk-granular: workers push whole request chunks
//! into a per-worker [`ChunkMailbox`] and the pool owner drains every
//! mailbox, so the free path never contends on a shared queue (experiment
//! 0357, `documentation/experiments/0357-chunk-mailbox-remote-free.md`,
//! confirmed by LOCUS-EVAL v1). [`RemoteFreeQueue`] is the earlier shared
//! bounded queue, kept as the comparison arm the benchmarks measure
//! against; LOCUS-EVAL v1 ranks it below every malloc baseline.

pub use crate::sys::chunk_mailbox::{ChunkMailbox, ChunkMailboxSender};
